...
public static class LinqProgram
{
    ...
    {
        return list.Where(value => value.Contains(pattern))
            .Order();
    }
    ...
    {
        return list.SkipWhile(value => !value.Equals("start"))
    ...
    {
        return list.Where(word => word.Length < 4)
            .Distinct()
            .ToList();
    }
    ...
    {
        return list.Count(value => value.StartsWith(startString));
    }
    ...
    {
        return words.Where(word => word.Contains("e"))
                .Order()
                .LastOrDefault()
            is { } lastWord
